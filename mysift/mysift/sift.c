...
static IplImage* convert_to_gray32( IplImage* img )
{
	...
	//gray32 = cvCreateImage(cvSize(1000,1000), IPL_DEPTH_32F, 1);//创建32位单通道图像
	gray32 = cvCreateImage(cvSize(img->width,img->height), IPL_DEPTH_32F, 1);//创建32位单通道图像
	...
	{
		gray8 = cvCreateImage(cvSize(img->width, img->height), IPL_DEPTH_8U, 1);//创建8位单通道图像
		//gray8 = cvCreateImage(cvSize(1000, 1000), IPL_DEPTH_8U, 1);//创建32位单通道图像
									...
									int intvls, double sigma )
{
    ...
    for( o = 0; o < octvs; o++ )//遍历组
        for( i = 0; i < intvls + 3; i++ )//遍历层
		{
            ...
            {   //创建图像
				gauss_pyr[o][i] = cvCreateImage(cvSize(gauss_pyr[o][i - 1]->width, gauss_pyr[o][i - 1]->height), IPL_DEPTH_32F, 1);
...
static IplImage*** build_dog_pyr( IplImage*** gauss_pyr, int octvs, int intvls )
{
    ...
    for( o = 0; o < octvs; o++ )//遍历组
        ...
        {   //创建DoG金字塔的第o组第i层的差分图像
			dog_pyr[o][i] = cvCreateImage(cvSize(gauss_pyr[o][i]->width, gauss_pyr[o][i]->height), IPL_DEPTH_32F, 1);
